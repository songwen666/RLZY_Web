...
export default {
  ...
  data() {
    ...
    // const validDescription = (rule, value, callback) => {
    //   // value !== null ? callback() : callback(new Error('不能为空'))
    //   // console.log('validDescription', value)
    //   return callback()
    // }
    return {
      showDialog: false,
      ...
      rules: {
        ...
        description: [
          { required: true, message: '权限介绍不能为空', trigger: 'blur' },
          // { validator: validDescription, trigger: 'blur' }
        ]
      }
    }
  },
