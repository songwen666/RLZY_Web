          ...
          <el-tab-pane label="组织架构" name="first">
            ...
            <el-row type="flex" justify="space-between" align="middle" class="department-header">
              <el-col :span="20">
                ...
                <span class="company">四川省万一教育科技股份有限公司</span>
              </el-col>
              <el-col :span="4">
                <!-- 在容器范围里, 一行再分2列 -->
...
export default {
  ...
  methods: {
    ...
    async getDepartmentsListFn() {
      ...
      // const re = await getDepartmentsListAPI()
      // console.log('@@@@@@获取部门列表信息', re)
      // console.log('出发了api')
    ...
    async getEmployeeSimpleFn() {
      ...
      // const re = await getEmployeeSimpleAPI()
      // const re = await getDepartmentsListAPI()
      // console.log('@@@@@@获取员工简洁信息', re)
      // console.log(res)
    ...
    async addDepartmentsFn(dataObj) {
      if (this.isEdit) { // 编辑状态
        ...
        // 添加状态 这里？ 这里没有id
        // console.log('asddasa', this.clickDepartId)
        dataObj.pid = this.clickDepartId
        // console.log('dataObj', dataObj)
        const ddd = await addDepartmentsAPI(dataObj)
        // console.log('ddd', ddd)
        if (!ddd.success) this.$message.error(ddd.message)
        this.$message.success(ddd.message)
      }
      this.getDepartmentsListFn()
    },
