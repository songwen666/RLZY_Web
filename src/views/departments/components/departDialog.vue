...
export default {
  ...
  data() {
    ...
    const validName = (rule, value, callback) => {
      let existNameList = this.originList.filter(item => item.pid === this.clickDepartId).map(item => item.name)
      console.log(existNameList, value)
      // 如果是编辑的情况
      if (this.isEdit) {
        // 找到部门的当前对象
        const dept = this.originList.find(item => item.id === this.clickDepartId)
        console.log('this.originList', this.originList)
        console.log('当前对象', dept)
        // 取出父级的 id (当前对象的pid)
        const pid = dept.pid
    ...
    const validCode = (rule, value, callback) => {
      ...
      if (this.isEdit) {
        existCodeList = this.originList.filter(item => item.id !== this.clickDepartId).map(item => item.code)
      }
