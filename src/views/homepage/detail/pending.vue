...
import { getUserPhotoAPI, getapprovalsPendByID } from '@/api'
...
export default {
  ...
  created() {
    ...
    this.getapprovalsPendByIDFn()
  },
  methods: {
    back() {
      this.$router.back()
    },
    async getapprovalsPendByIDFn() {
      const res = await getapprovalsPendByID(this.userId)
