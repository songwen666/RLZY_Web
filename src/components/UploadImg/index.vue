<template>
  <!-- action文件上传地址 -->
  <!-- http-request 使用过后 便不会再执行action的内容 -->
  <el-upload
  ...
  >
    ...
    <el-progress v-show="showProgress" type="circle" :percentage="parseInt(fake.progress * 100)" class="progress" :color="colors" />
  </el-upload>
</template>
      ...
      if (res.file) {
        ...
        cos.putObject({
          ...
          onProgress: function(progressData) {
            // console.log('progressData', progressData.percent)
            if (progressData.percent === 0) {
              this.fake.progress = 0
            } else if (parseInt(this.fake.progress * 100) === 99) {
              this.fake.end()
            }
          }
        }, (err, data) => {
